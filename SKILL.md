...
description: |
  [每回合强制] HANCLI docs 维护 + 问答归档。Skill 在 available_skills 即生效：用户问本项目技术问题（说明/如何/MCP/CLI/Skill…）→ Agent 须先 Write docs/questions/ 再回复；禁止等用户说「归档」。答复前必读 references/every-turn-protocol.md。Cursor 不代写文件。
version: "1.9.0"
author: HanRuo
...
用户**不必**说「归档 / doc-maintenance / docs/questions」。Rule：`.cursor/rules/doc-maintenance-qna-archive.mdc`（`alwaysApply: true`）。
...
| **本会话已读过协议或 Rule** | 读过 `every-turn-protocol.md`、`SKILL.md` 或 `.cursor/rules/doc-maintenance-qna-archive.mdc` |
| **Rule 已 alwaysApply** | 本仓库已配置 `doc-maintenance-qna-archive.mdc`（默认 true） |
...
| `description` / Rule | 显式写「先 Write 再回复」 |
...
1. 复制整个 `doc-maintenance/` 到 `.cursor/skills/`（或 `.hancli/skills/` 供 HANCLI CLI）。
2. 复制 `.cursor/rules/doc-maintenance-qna-archive.mdc`（**必须**，否则 Cursor 仍易跳过落盘）。
3. 确认 `references/every-turn-protocol.md` 一并复制（v1.9.0 每回合必读协议）。
4. 仅改 `references/project-profile.md`；`/skill reload` 或重启 Agent 会话。
5. 新项目首次 load 时走 § 一、§ 二 自动检测。
