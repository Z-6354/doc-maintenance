...
description: |
  [每回合强制 · 项目级 Skill] 通用 docs 维护 + 问答归档。兼容 Cursor、Claude Code、Codex、Hermes 等支持 SKILL.md 的 Agent：用户问本项目技术问题 → 须先 Write docs/questions/ 再回复；禁止等用户说「归档」。答复前必读 references/every-turn-protocol.md。Agent 运行时不会代写文件。
version: "1.11.0"
author: HanRuo
tags: [docs, markdown, maintenance, reusable, qna-archive, multi-agent]
---
...
用户**不必**说「归档 / doc-maintenance / docs/questions」。**仅依赖本 Skill 被宿主加载**（见 [`references/agent-runtime.md`](references/agent-runtime.md)）。

### 问答自动归档（强制摘要）

本仓库启用 **doc-maintenance**（Skill 被 Agent 加载即生效）。**任何 Agent 运行时均不会**在对话结束后自动写 `docs/questions/`。
...
**回合结束强制门**见 [§ 零](#零回合结束强制门agent-运行时最高优先级)；检查清单见 `references/agent-completion-gate.md`；Agent 兼容见 `references/agent-runtime.md`。

---

## 零、回合结束强制门（Agent 运行时，最高优先级）

> **「自动归档」** 在本 Skill 中指：**Agent 在同一回合内必须用写文件工具落盘**；**不是** IDE/Agent 平台在对话结束后自动写 `docs/questions/`。  
...
| 信号 | 说明 |
|------|------|
| **Skill 已被宿主加载** | 出现在 Agent 的 skill 列表 / system prompt / 用户 `@doc-maintenance` 或等价引用（详见 `agent-runtime.md`） |
...
| 本 Skill / Agent 机制 | Skill 与 Rule/Hook 区别、为何不会自动写盘（**仍要归档该次 Q&A**） |
...
### 0.5 v1.11.0 相对 v1.10.0 的变更

| 变更 | 说明 |
|------|------|
| `references/agent-runtime.md` | 新增：Cursor / Claude Code / Codex / Hermes 等安装路径与生效信号 |
| 全文去 Cursor 绑定 | 「Cursor 不代写」→「Agent 运行时不会代写」；路径改为多 Agent 表 |
| § 九 / README | 安装说明通用化，`{SKILLS_DIR}` 占位 |

### 0.6 v1.10.0 相对 v1.9.0 的变更
...
与 [§ 零](#零回合结束强制门agent-运行时最高优先级) 绑定；**未执行步骤 4～7 不得结束回合**。
...
1. 克隆整个 `doc-maintenance/` 到宿主 **Skill 目录**（见 [`references/agent-runtime.md`](references/agent-runtime.md)）：
   - Cursor → `.cursor/skills/doc-maintenance/`
   - Claude Code → `.claude/skills/doc-maintenance/`
   - Hermes / 通用 → `.skills/doc-maintenance/`
   - HANCLI CLI → `.hancli/skills/doc-maintenance/`（可与 IDE 路径并存）
2. 确认 `references/every-turn-protocol.md` 与 `references/agent-runtime.md` 一并存在。
3. 仅改 `references/project-profile.md`；重载 Skill 或重启 Agent 会话。
