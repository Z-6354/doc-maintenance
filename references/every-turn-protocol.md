...
> 配合 `SKILL.md` § 零、§ 七；检查清单见 `agent-completion-gate.md`。
...
|----|------|------|
| **1** | 检索源码 / 文档，**构思完整答复** | — |
| **2** | 调用 **Write / StrReplace** 落盘 `docs/questions/`（§7.3～7.5） | 未写文件就向用户发最终答复 |
...
Cursor **不会**在后台代写 `docs/questions/`；「自动归档」= Agent **同回合**写文件，且**无需用户确认**。
...
| 应归档（默认） | 不归档 |
|----------------|--------|
| 说明 MCP / CLI / 架构 / Skill / 配置 | 与 HANCLI 仓库无关 |
