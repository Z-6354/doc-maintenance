...
> 配合 `every-turn-protocol.md` 与 `SKILL.md` **§ 0.0、§ 零**。宿主加载 Skill ≠ 已归档；**Write/StrReplace**（或等价写文件工具）才算完成。
...
- [ ] `doc-maintenance` 已被宿主 Agent 加载，或已读 `every-turn-protocol.md` / SKILL 文首「首读」（见 [`agent-runtime.md`](agent-runtime.md)）

**是** → 下节起默认**须归档**；用户**无需**说「归档」「写 docs」。  
**否** → 仅当用户明确触发 doc-maintenance 时再归档。

## 2. 本回合是否须归档？

- [ ] 用户问题与 **本仓库**（见 `project-profile.md`：源码、docs、CLI、配置等）相关
